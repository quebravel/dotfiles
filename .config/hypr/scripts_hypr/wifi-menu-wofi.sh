...
## This script is based on Zach Baylin's wofi WiFi Menu.
...
CHENTRY=$(echo -e "$MENU" | wofi -dmenu -p "WiFi SSID" -c)
CHSSID=$(echo "$CHENTRY" | sed  's/\s\{2,\}/\|/g' | awk -F "|" '{print $1}')

## Support manual SSID entry.
if [ "$CHENTRY" = "manually connect to a network" ] ; then
	MSSID=$(echo "Enter your network's SSID." | wofi -dmenu -p "SSID: " -lines 1)
	WIFI_PASS=$(echo "Enter the network password." | wofi -dmenu -password -p "Password: " -lines 1)
